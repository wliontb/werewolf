<template>
    <div class="flex flex-col w-full gap-4">
        <div class="flex flex-col gap-2">
            ...
            <div class="flex">
                <div class="w-1/2">Hiện tại đang có: <span class="text-red-500 font-semibold">{{ gameStore.totalPlayer
                        }}</span> người chơi</div>
                <div class="flex w-1/2 gap-1">
                    <input type="text" class="w-4/6 p-1.5 rounded text-black border-white border" placeholder="điền tên người chơi và enter"
                        v-model="playerNameInput" @keyup.enter="addPlayer">
                    <button
                        class="w-1/6 text-xs flex items-center justify-between bg-green-500 rounded p-1.5 uppercase hover:bg-green-700 border-white border"
                        @click="addPlayer">Thêm người</button>
                    <button
                        class="w-1/6 text-xs flex items-center justify-between bg-red-500 rounded p-1.5 uppercase hover:bg-red-700 border-white border"
                    ...
                    <tbody class="bg-white text-black font-semibold">
                        <tr v-for="player in playerStore.player" class="hover:bg-green-300 border-b border-dashed">
                            ...
                            <td class="py-3 pl-8 text-center">
                                <button class="bg-red-500 text-white rounded p-1 text-xs uppercase" title="Xóa người chơi này"
                                    @click="playerStore.remove(player.id)">
                                    <IconTrash />
                                </button>
                            </td>
                        </tr>
